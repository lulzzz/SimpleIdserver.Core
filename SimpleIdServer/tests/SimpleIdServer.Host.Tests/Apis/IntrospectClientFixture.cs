...
namespace SimpleIdServer.Host.Tests.Apis
{
    public class IntrospectClientFixture : IClassFixture<TestOauthServerFixture>
    {
        ...
        public async Task When_Introspect_And_Client_Not_Authenticated_Then_Error_Is_Returned()
        {
            ...
            Assert.Equal((string) "invalid_client", (string) introspection.Error.Error);
            Assert.Equal((string) "the client doesn't exist", (string) introspection.Error.ErrorDescription);
        }
        ...
        public async Task When_Introspect_And_Token_Doesnt_Exist_Then_Error_Is_Returned()
        {
            ...
            Assert.NotNull(introspection);
            Assert.True(introspection.ContainsError);
            Assert.Equal((string) "invalid_token", (string) introspection.Error.Error);
            Assert.Equal((string) "the token is not valid", (string) introspection.Error.ErrorDescription);
        }
