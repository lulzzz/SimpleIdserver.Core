...
namespace SimpleIdServer.Host.Tests.Apis
{
    public class UserInfoClientFixture : IClassFixture<TestOauthServerFixture>
    {
        ...
        public async Task When_Pass_Invalid_Token_To_UserInfo_Then_Error_Is_Returned()
        {
            ...
            Assert.Equal((string) "invalid_token", (string) getUserInfoResult.Error.Error);
            Assert.Equal((string) "the token is not valid", (string) getUserInfoResult.Error.ErrorDescription);
        }
        ...
        public async Task When_Pass_Client_Access_Token_To_UserInfo_Then_Error_Is_Returned()
        {
            ...
            Assert.True(getUserInfoResult.ContainsError);
            Assert.Equal((string) "invalid_token", (string) getUserInfoResult.Error.Error);
            Assert.Equal((string) "not a valid resource owner token", (string) getUserInfoResult.Error.ErrorDescription);
        }
