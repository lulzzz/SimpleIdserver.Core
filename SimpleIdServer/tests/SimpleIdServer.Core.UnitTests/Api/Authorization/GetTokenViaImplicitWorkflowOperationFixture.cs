...
namespace SimpleIdentityServer.Core.UnitTests.Api.Authorization
{
    public class GetTokenViaImplicitWorkflowOperationFixture
    {
        ...
        public async Task When_Passing_No_Nonce_Parameter_Then_Exception_Is_Thrown()
        {
            ...
            // ACT & ASSERTS
            var exception = await Assert.ThrowsAsync<IdentityServerExceptionWithState>(() => _getTokenViaImplicitWorkflowOperation.Execute(authorizationParameter, new Client(), null));
        ...
        public async Task When_Implicit_Flow_Is_Not_Supported_Then_Exception_Is_Thrown()
        {
            ...
            // ACT & ASSERTS
            var ex = await Assert.ThrowsAsync<IdentityServerExceptionWithState>(() => _getTokenViaImplicitWorkflowOperation.Execute(authorizationParameter, new Client(), null));
        ...
        public async Task When_Requesting_Authorization_With_Valid_Request_Then_Events_Are_Logged()
        {
            ...
            _processAuthorizationRequestFake.Setup(p => p.ProcessAsync(It.IsAny<AuthorizationParameter>(), It.IsAny<Client>(), null, "fake", 1212)).Returns(Task.FromResult(actionResult));
            _clientValidatorFake.Setup(c => c.CheckGrantTypes(It.IsAny<Client>(), It.IsAny<GrantType[]>()))
                .Returns(true);

            // ACT
            await _getTokenViaImplicitWorkflowOperation.Execute(authorizationParameter, new Client(), null, "fake", 1212);
